...
function spinner(label: string) {
	...
	const id = setInterval(() => {
		process.stdout.write(
			`\r${paint(c.cyan, frames[i++ % frames.length])} ${paint(c.dim, label)}   `,
		);
	}, 80);
	return () => {
		clearInterval(id);
...
async function readFile(filePath: string): Promise<string> {
	console.log(
		paint(c.yellow, `  📄 read_file`) + paint(c.dim, ` ${filePath}`),
	);
	try {
...
async function writeFile(filePath: string, content: string): Promise<string> {
	console.log(
		paint(c.green, `  ✏️  write_file`) + paint(c.dim, ` ${filePath}`),
	);
	try {
...
async function editFile(
	...
	console.log(
		paint(c.magenta, `  🔧 edit_file`) + paint(c.dim, ` ${filePath}`),
	);
	try {
		const original = await fs.readFile(filePath, "utf-8");
		if (!original.includes(oldStr))
			return `edit_file: old_str not found in ${filePath}`;
...
async function listDirectory(
	...
	console.log(
		paint(c.blue, `  📁 list_directory`) + paint(c.dim, ` ${dirPath}`),
	);
	try {
		const flag = recursive ? "-R" : "";
		const { stdout } = await execAsync(`ls ${flag} ${dirPath}`, {
			timeout: 5000,
		});
		return stdout || "(empty)";
	} catch (err) {
...
async function searchFiles(pattern: string, directory = "."): Promise<string> {
	console.log(
		paint(c.cyan, `  🔍 search_files`) +
			paint(c.dim, ` "${pattern}" in ${directory}`),
	);
	try {
...
async function runCommand(command: string): Promise<string> {
	console.log(
		paint(c.yellow, `  🖥️  run_command`) + paint(c.dim, ` ${command}`),
	);
...
const declarations: FunctionDeclaration[] = [
	...
	},
	{
		name: "edit_file",
		description:
			"Surgically replace an exact string in a file. Fails if old_str is not found.",
		parameters: {
			type: SchemaType.OBJECT,
			properties: {
...
function banner() {
	...
	console.log(
		paint(c.cyan + c.bold, "  ║") +
			paint(c.white + c.bold, "     ⚡ VibeCode Agent          ") +
			paint(c.cyan + c.bold, "║"),
	);
	console.log(
		paint(c.cyan + c.bold, "  ║") +
			paint(c.dim, "   Gemini 2.5 Flash · TS CLI  ") +
			paint(c.cyan + c.bold, " ║"),
	);
	console.log(paint(c.cyan + c.bold, "  ╚══════════════════════════════╝"));
...
async function main() {
	...
	while (true) {
		const userQuery = await rl.question(
			paint(c.green + c.bold, "you ❯ ") + c.reset,
		);
		...
		try {
			...
			console.log();
			console.log(
				paint(c.magenta + c.bold, "vibe ❯ ") + result.response.text(),
			);
			console.log();
		} catch (err) {
			stop();
