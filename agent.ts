...
async function getWeather(city: string): Promise<string> {
	...
	try {
		...
		if (!res.ok)
			return `Could not fetch weather for ${city} (status ${res.status}).`;
	...
	},
	{
		...
		parameters: {
			...
			properties: {
				owner: {
					type: SchemaType.STRING,
					description: "Repo owner, e.g. 'whomimohshukla'.",
				},
				repo: {
					type: SchemaType.STRING,
					description: "Repo name, e.g. 'quickfixo'.",
				},
				path: {
					type: SchemaType.STRING,
				...
				branch: {
					type: SchemaType.STRING,
					description:
						"Target branch, e.g. 'main'. Optional, defaults to repo default branch.",
				},
			},
